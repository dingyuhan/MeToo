      ...
      <div class=" col-md-2 sidebar">
        <div id="accordion">
    	  ...
    	  <div class="table-responsive ">
    	    <h3 style="display:inline;">积分兑换</h3>
          <form action="/MeToo/index.php/Admin/Personal/addexchange" method="post" style="display:inline; margin-left:1100px;">
            <button type="submit" class="btn btn-warning">添加积分兑换</button>
          </form>
          <table width="100%" border="0" cellspacing="0" cellpadding="0"  class="table  table-striped table-hover " style="margin-top:20px;">
            ...
            <?php if(is_array($exchange)): $i = 0; $__LIST__ = $exchange;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$exchange): $mod = ($i % 2 );++$i;?><tr>
              <th width="4%"><input type="checkbox" name="checkbox10" id="checkbox10"></th>
    	        <td><?php echo ($exchange["id"]); ?></td>
    	        <td><?php echo ($exchange["goods"]); ?></td>
              <td><?php echo ($exchange["thumb"]); ?></td>
    	        <td><?php echo ($exchange["gnum"]); ?></td>
              <td><?php echo ($exchange["gmark"]); ?></td>
    	        <td><a href="deleteexchange/id/<?php echo ($exchange["id"]); ?>">删除</a><a href="modiexchange/id/<?php echo ($exchange["id"]); ?>">修改</a></td>
